...
describe Refinery::PageImages::Engine do
  ...
  describe 'attach initializer' do
    it 'calls attach on all configured model' do
      Refinery::PageImages.config.stub(:enable_for)
        .and_return(enable_for_config)

      Refinery::PageImages::EnableForMock::Model
        .should_receive(:many_page_images?).once
      Refinery::Page.should_not_receive(:many_page_images?)
